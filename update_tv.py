...
# Flux complet et stable
XMLTV_URL = "https://github.com/thegoodb/fr/raw/main/spetnazfr.xml"

# Configuration simplifiée par mots-clés (Texte recherché -> Canal, Nom propre, Source)
CHAINES_CONFIG = {
    # --- TNT & FREEBOX (Grille Nouvelle Génération) ---
    ...
    "BFM TV": ("15", "BFM TV", "Freebox / Molotov"),
    "CNEWS": ("16", "CNews", "Freebox / Molotov"),
    "CSTAR": ("17", "CStar", "Freebox / Molotov"),
    "T18": ("18", "T18", "Freebox / Molotov"),                  # Nouvelle chaîne Canal 18
    "SERIES": ("19", "TF1 Series Films", "Freebox / Molotov"),
    "EQUIPE": ("20", "L'Equipe", "Freebox / Molotov"),
    "6TER": ("21", "6ter", "Freebox / Molotov"),
    "RMC STORY": ("22", "RMC Story", "Freebox / Molotov"),
    "RMC DECOU": ("23", "RMC Découverte", "Freebox / Molotov"),
    "NOVO": ("24", "NOVO", "Freebox / Molotov"),                 # Nouvelle chaîne Canal 24
    "OUEST": ("25", "Ouest-France TV", "Freebox / Molotov"),
    "LCI": ("26", "LCI", "Freebox / Molotov"),
    "FRANCEINFO": ("27", "Franceinfo", "Freebox / Molotov"),

    # --- THEMATIQUES FREEBOX ---
    "PARIS PREM": ("28", "Paris Première", "Freebox TV"),
    ...
    "GAME ONE": ("118", "Game One", "Freebox TV"),
    "MANGAS": ("121", "Mangas", "Freebox TV"),
    "HISTOIRE": ("205", "Histoire TV", "Freebox TV"),
    "SCIENCE": ("207", "Science & Vie TV", "Freebox TV"),
    "USHUAIA": ("204", "Ushuaïa TV", "Freebox TV"),

    # --- SAMSUNG TV PLUS ---
    "COMEDY": ("4124", "Comedy Central", "Samsung TV Plus"),
    "PLUTO CINE": ("4142", "Pluto TV Ciné", "Samsung TV Plus"),
    "RAKUTEN ACT": ("4112", "Rakuten TV Action", "Samsung TV Plus"),
    "DOCTOR WHO": ("4304", "Doctor Who TV", "Samsung TV Plus"),
    "PLUTO SERI": ("4145", "Pluto TV Séries", "Samsung TV Plus")
}
...
def main():
    ...
    try:
        ...
        for prog in root.findall('programme'):
            ...
            if start_time and start_time.startswith(aujourdhui):
                ...
                nom_xml = dict_chaines.get(chan_id, "")
    ...
    if not programmes_filtres:
        programmes_filtres.append({
            "canal": "0", "heure": "00:00", "chaine": "Aide", 
            "titre": "Aucun programme trouvé aujourd'hui.", 
    ...
    print(f"Réussite : {len(programmes_filtres)} programmes sauvegardés.")
